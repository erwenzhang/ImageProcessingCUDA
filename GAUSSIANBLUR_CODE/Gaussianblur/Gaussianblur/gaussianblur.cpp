...
template <class T> bool TGaussianBlur<T>::CExtension::_Allocate(const CSize &_Size, unsigned int _Margin)
{
    ...
    TGaussianBlur<T>::CArray::Buffer = new T[(_Size.x > _Size.y ? _Size.x : _Size.y) + (_Margin << 1)];
	//   Check buffer allocation
	if (!TGaussianBlur<T>::CArray::Buffer )
		//   Buffer allocation failed
		return false;
	//   Initialize size descriptors
	TGaussianBlur<T>::CArray::Size = _Size;
	Margin = _Margin;
	//   Succeeded
	return true;
}
...
template <class T> void TGaussianBlur<T>::CExtension::Paste(const T *const _Start)
{
	...
	{
		//   Paste line
        memcpy(TGaussianBlur<T>::CArray::Buffer + Margin, _Start, TGaussianBlur<T>::CArray::Size.x * sizeof(T));
	}
	else
	{
		//   Stop position
		const T *const Stop = _Start + TGaussianBlur<T>::CArray::Size.Area();
		//   Array iterator
		const T *ArrIter = _Start;
		//   Extension iterator
		T *ExtIter = TGaussianBlur<T>::CArray::Buffer + Margin;
		...
		while (ArrIter < Stop)
		{
			...
			*(ExtIter++) = *ArrIter;
			...
			ArrIter += TGaussianBlur<T>::CArray::Size.x;
		}
	}
}
...
{
	//   Line size
	const unsigned int Line = Mode == ModeHorizontal ? TGaussianBlur<T>::CArray::Size.x : TGaussianBlur<T>::CArray::Size.y;
	//   Stop position
	const T *const Stop = TGaussianBlur<T>::CArray::Buffer - 1;
	//   Left extension iterator
	T *ExtLeft = TGaussianBlur<T>::CArray::Buffer + Margin - 1;
	//   Left array iterator
