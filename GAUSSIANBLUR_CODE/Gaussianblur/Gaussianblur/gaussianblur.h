...
protected:
	...
	struct CArray
	{
        ...
        ~CArray() { if(Buffer) delete[] Buffer; }
...
//		~CExtension() { if(Buffer) delete[] Buffer; }
	...
	struct CWindow
	{
		...
		//   Default constructor
		CWindow(): Weights(nullptr), Size(0), Correction(0) {}
		//   Destructor
		...
		T Apply(const T *_Element) const
		{
            ...
            while (WeightIter < End){
                SumR += *(WeightIter) * (double)((*ElIter).r);
                SumG += *(WeightIter) * (double)((*ElIter).g);
                SumB += *(WeightIter++) * (double)((*ElIter++).b);
            }
			return T(SumR,SumG,SumB);
		}
